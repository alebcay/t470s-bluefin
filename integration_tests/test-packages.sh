...
# Use -n to avoid prompting for password in non-interactive SSH; wheel is NOPASSWD via 99-wheel-nopasswd but requiretty may block.
if sudo -n bootc status >/dev/null 2>&1; then
	echo 'OK: bootc status succeeded'
elif sudo bootc status >/dev/null 2>&1; then
	echo 'OK: bootc status succeeded'
else
	# In VM without tty, sudo may still fail; try without sudo if bootc is readable, or just warn.
	if bootc status >/dev/null 2>&1; then
		echo 'OK: bootc status succeeded without sudo'
	else
